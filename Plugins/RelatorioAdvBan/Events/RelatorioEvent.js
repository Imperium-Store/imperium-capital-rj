...
module.exports = {
    ...
    async execute(interaction) {
        if (interaction.isModalSubmit()) {
            if (interaction.customId === "advertencia-modal-1") {
                ...
                const tempData = interaction.client.tempData.get(
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        {
                            name: "Itens Looteados",
                            value: itensLooteados,
                            inline: false,
                        },
                        {
                ...
                const mentionRoles = config.commandRequireRole
                    .map((roleId) => `<@&${roleId}>`)
                    .join(", ");
        ...
        if (interaction.isButton()) {
            const tempData = interaction.client.tempData.get(interaction.channel.id);
  ...
  **PROVAS:** ${data.Provas}
            `;
                ...
                await channelLogsAdm.send({
                    embeds: [embed],
                });

                if (mappedObject.ItensLooteados !== "Nenhum") {
