...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const createTranscriptAndSend = async (client, channel, transcriptBuffer, config, ticketCreator, uniqueUsers) => {
            const transcriptChannel = await client.channels.fetch(config.transcript_log_channel);
        ...
        if (interaction.type === InteractionType.MessageComponent) {
            if (interaction.componentType === ComponentType.StringSelect && interaction.customId === "selectTicketType") {
                ...
                const channel = await guild.channels.create({
                    ...
                    permissionOverwrites: createChannelPermissions(guild, interaction.user.id, ticketConfig.allowed_roles)
                });

                await channel.send({
                    content: `<@${interaction.user.id}> ${ticketConfig.allowed_roles.map(roleID => `||<@&${roleID}>||`).join(" ")}`,
